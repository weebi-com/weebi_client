...
class BoutiqueCreateView extends StatefulWidget {
  ...
  /// Create a new chain
  const BoutiqueCreateView.createChain({
    super.key,
    this.formExtensions,
  })  : boutique = null,
        chain = null,
...
class _BoutiqueCreateViewState extends State<BoutiqueCreateView> {
  ...
  void initState() {
    super.initState();
    _initializeForm();
    _notifyFormExtensionsReady();
  }

  void _notifyFormExtensionsReady({Chain? parentChain}) {
    ...
    if (widget.isEditing) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (parentChain != null) {
          ...
          setState(() {});
          _notifyFormExtensionsReady(parentChain: parentChain);
        } else {
          _notifyFormExtensionsReady();
        }
      });
                  ...
                  onChanged: (value) {
                    setState(() {
                      ...
                      if (!widget.isEditing) {
                        ...
                        widget.formExtensions?.onParentChainSelected
                            ?.call(chain);
                      }
                    });
                  },
